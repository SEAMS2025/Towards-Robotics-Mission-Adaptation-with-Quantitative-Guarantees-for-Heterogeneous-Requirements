...
class PrismModelGenerator:
    ...
    def _generate_transitions(self, x: int, y: int, g1: bool, g2: bool, g3: bool, 
        ...
        for i, (dx, dy) in enumerate(moves):
            if int_probs[i] > 0:
                ...
                # Update goal flags with proper sequencing
    ...
    def generate_prism_model(self, policy: Dict[Tuple[int, int, bool, bool, bool], int],
        ...
        # Add labels for properties including both obstacles
    ...
    def verify_property(self, model_str: str, property_str: str) -> List[float]:
        ...
        try:
            ...
            cmd = [
                ...
                "-explicit",
                "-javamaxmem", "4g",
                "-maxiters", "1000000",  # Increased iteration limit
                "-power", # Use power method for numerical computation
                "-verbose",
                "-exportstates", "states.txt"  # Export states for debugging
            ]
...
class LTLGuidedQLearning:
    """Q-Learning with LTL guidance for grid world navigation"""
    ...
    def _update_prism_probabilities(self, probabilities: List[float]):
        """Update stored PRISM verification probabilities"""
    ...
    def _calculate_reward(self, state: tuple, next_state: tuple) -> float:
        """Calculate reward with LTL guidance"""
        x, y, g1, g2, g3 = state
        ...
        if current_goal:
            ...
            progress_reward = -0.01 * dist_to_goal
            base_reward += progress_reward

        if not g1 and next_x == 5 and next_y == 8:
            base_reward += 100.0 * (1 + self.prism_probs['goal1'])
        elif g1 and not g2 and next_x == 4 and next_y == 2:
            base_reward += 100.0 * (1 + self.prism_probs['goal2'])
        elif g2 and not g3 and next_x == 9 and next_y == 9:
            base_reward += 1000.0 * (1 + self.prism_probs['goal3'])
        ...
        try:
            ...
            for episode in range(episodes):
                ...
                while not done:
                    ...
                    ltl_reward = self._calculate_reward(state, next_state)
                    
                    current_q = self.q_table[state][action]
                    next_max_q = np.max(self.q_table[next_state])
                    new_q = current_q + self.alpha * (
                        ltl_reward + self.gamma * next_max_q - current_q
                    )
