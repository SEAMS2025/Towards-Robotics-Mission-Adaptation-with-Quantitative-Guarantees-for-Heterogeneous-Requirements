...
class LTLGuidedQLearningWithObstacle:
            ...
            for y in range(self.size):
                for g1 in [False, True]:
                    for g2 in [False, True]:
                        for g3 in [False, True]:
                            state = (x, y, g1, g2, g3)
                            self.q_table[state] = np.ones(self.action_space) * 1.0  # Optimistic initialization

    def _update_prism_probabilities(self, probabilities: List[float]):
        """Update PRISM verification probabilities"""
    ...
    def _calculate_sequence_weight(self, state: tuple, next_state: tuple) -> float:
        """Calculate weight based on sequence probabilities"""
        _, _, g1, g2, g3 = state
    ...
    def _calculate_counterfactual_reward(self, state: tuple, next_state: tuple, k: int) -> float:
        """Reward calculation with better balance"""
        x, y, g1, g2, g3 = state
        next_x, next_y, next_g1, next_g2, next_g3 = next_state
        
        base_reward = -0.05 * (1 + 0.05 * self.size/4)  
        ...
        if (next_x, next_y) in [(6, 6), (3, 4)]:  
            ...
            dist_to_obstacle1 = abs(next_x - 6) + abs(next_y - 6)
            dist_to_obstacle2 = abs(next_x - 3) + abs(next_y - 4)
            min_dist = min(dist_to_obstacle1, dist_to_obstacle2)
            if min_dist < 3:  
        ...
        current_goal = None
        if not g1:
            current_goal = (5, 8)  # Goal 1
        elif not g2:
            current_goal = (4, 2)  # Goal 2
        elif not g3:
            current_goal = (9, 9)  # Goal 3
        ...
        if not g1 and next_x == 5 and next_y == 8:
        ...
        elif g2 and not g3 and next_x == 9 and next_y == 9:
            ...
            penalty_factor = 1.0 + (1 - min(self.prism_probs['seq1'],
                                        self.prism_probs['seq2'],
                                        self.prism_probs['seq3']))
            base_reward -= 10.0 * penalty_factor  
    ...
    def _get_counterfactual_states(self, state: tuple, next_state: tuple) -> List[Tuple]:
        ...
        for goal_x, goal_y in goals:
            if abs(x - goal_x) + abs(y - goal_y) <= 2: 
                for alt_g1 in [False, True]:
                    for alt_g2 in [False, True]:
                        for alt_g3 in [False, True]:
                            if (not alt_g2 or alt_g1) and (not alt_g3 or alt_g2):
                                current_state = (x, y, alt_g1, alt_g2, alt_g3)
                                next_alt_g1 = alt_g1 or (next_x == 5 and next_y == 8)
                                next_alt_g2 = alt_g2 or (next_alt_g1 and next_x == 4 and next_y == 2)
                                next_alt_g3 = alt_g3 or (next_alt_g2 and next_x == 9 and next_y == 9)
                                next_cf_state = (next_x, next_y, next_alt_g1, next_alt_g2, next_alt_g3)
                                counterfactuals.append((current_state, next_cf_state))
            ...
            for alt_g2 in [False, True]:
                for alt_g3 in [False, True]:
                    if (not alt_g2 or alt_g1) and (not alt_g3 or alt_g2):
                        ...
                        next_alt_g3 = alt_g3 or (next_alt_g2 and next_x == 9 and next_y == 9)
                        next_cf_state = (next_x, next_y, next_alt_g1, next_alt_g2, next_alt_g3)
                        if (current_state, next_cf_state) not in counterfactuals:
                            counterfactuals.append((current_state, next_cf_state))
    ...
    def _adjust_parameters(self, ltl_score: float):
        ...
        if ltl_score > self.best_ltl_score:
            ...
            self.epsilon = min(0.8, self.epsilon * 1.02)
...
def main():
    ...
    try:
        ...
        logger.info(f"Goals: G1(5,8), G2(4,2), G3(9,9), Obstacle(6,6)")
        ...
        agent.verifier.save_probabilities_to_file(agent, results_dir)  # Add this line
